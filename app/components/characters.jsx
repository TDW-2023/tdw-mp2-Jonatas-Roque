...
export default function CharactersPage() {
  ...
  if (isLoading) {
    return <div className="mx-auto text-center pt-[20vh] h-screen">Loading...</div>;
  }

  if (error) {
    return (
      <div className="mx-auto text-center pt-[20vh] h-screen">Error: {error.message}</div>
    );
  }
  ...
  const filteredCharacters = characters.results.filter((item) =>
  item.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
    (selectedGender === '' || item.gender.toLowerCase() === selectedGender.toLowerCase()) &&
    (selectedSpecies === '' || speciesMapping[item.species] === selectedSpecies)
);
  ...
  return (
    ...
    <div className="w-3/4 mx-auto text-center mt-[10vh]">
      <h1 className="mx-auto text-center text-3xl font-bold mb-12">Star Wars Characters</h1>
    ...
    <div className="flex justify-between">
        ...
        <input
          ...
          <div className="text-black">
            <select className="mx-3 py-2 px-4 bg-[#1a1a1a] text-white" onChange={(e) => setSelectedGender(e.target.value)} value={selectedGender}>
            ...
            <select className="mx-3 py-2 px-4 bg-[#1a1a1a] text-white" onChange={(e) => setSelectedSpecies(e.target.value)} value={selectedSpecies}>
    ...
    {filteredCharacters.length > 0 ? (
      filteredCharacters.map((item, index) => {
        ...
        return(
          <div className="hover:transition hover:ease-in transition ease-in-out">
          <Image
          ...
          <div key={index} className="bg-[#1a1a1a]">
            ...
            <Link href={`/characterDetails/?id=${number}`} >
              <div className="w-50 bg-transparent cursor-pointer border-[#ffc107] text-center py-2 text-[#ffc107] hover:bg-[#ffc107] hover:text-black border hover:transition hover:ease-in-out transition ease-in">About{/*lINK*/}
              </div>
              </Link>
      ...
      <div className="text-center mx-auto">No matching characters found.</div>
    )}
